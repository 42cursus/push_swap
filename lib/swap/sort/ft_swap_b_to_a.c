...
{
	while (pswap->stack_b_size
		&& pswap->stack_b[pswap->top_b] == pswap->sorted[pswap->top_a - 1])
	{
		if (pswap->pivots_b_tab_size > 0
			&& pswap->stack_b[pswap->top_b] == pswap->pivots_b[0])
...
int	ft_swap_get_curr_pivot_b(t_pswap *pswap)
{
	...
	{
		curr_pivot = ft_swap_get_pivot(pswap, 'b',
				pswap->top_b, pswap->arg_tab_size - 1);
